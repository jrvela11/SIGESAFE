...
import {
  Plus, Edit, Trash2, Search, X, Save, Phone,
  RefreshCw, ChevronLeft, ChevronRight,
  CreditCard, Truck, CheckCircle, Ban
} from "lucide-react";
...
export const TransportistasView = () => {
  const pt = useTransportistas();

  const Paginador = () => {
    if (pt.totalPaginas <= 1) return null;
    return (
      <div className="flex items-center gap-1.5">
        <button
          onClick={() => pt.setPagina((p) => Math.max(1, p - 1))}
          disabled={pt.paginaAjustada === 1}
        ...
        {Array.from({ length: pt.totalPaginas }, (_, i) => i + 1).map((n) => (
          <button
            key={n}
            onClick={() => pt.setPagina(n)}
        ...
        <button
          onClick={() => pt.setPagina((p) => Math.min(pt.totalPaginas, p + 1))}
          disabled={pt.paginaAjustada === pt.totalPaginas}
          className="w-7 h-7 rounded-md bg-white border border-[#EDE8E1] flex items-center justify-center text-[#7A6E65] hover:bg-[#F7F5F2] disabled:opacity-40 disabled:cursor-not-allowed transition-colors"
        >
    ...
    <DashboardLayout>
      <div className="space-y-5 pb-12">
        ...
        <div className="flex items-start justify-between">
          <div>
            ...
            <p className="text-[12.5px] text-[#8B7D72] mt-1">
              Administra las empresas y personas encargadas de tus envíos y logística.
            </p>
          </div>
          <button
            onClick={pt.abrirModalCrear}
        ...
        <div className="grid grid-cols-1 sm:grid-cols-3 gap-3">
          {[
            {
              icon: <Truck className="w-5 h-5 text-[#C17B2A]" />,
              iconBg: "bg-[#FDF3E7]",
              value: pt.totalTransportistas,
              label: "Total registrados",
            },
            {
              icon: <CheckCircle className="w-5 h-5 text-[#0D7A3E]" />,
              iconBg: "bg-[#EDFBF3]",
              value: pt.transportistasActivos,
              label: "Transportistas activos",
            },
            {
              icon: <Phone className="w-5 h-5 text-[#1A5FA0]" />,
              iconBg: "bg-[#E6F1FB]",
              value: pt.transportistasConTelefono,
        ...
        <div className="flex flex-col sm:flex-row items-start sm:items-center gap-3">
          <div className="flex-1 flex items-center gap-2 bg-white border border-[#EDE8E1] rounded-lg px-3 py-2 focus-within:border-[#C17B2A] focus-within:ring-2 focus-within:ring-[#C17B2A]/15 transition-all w-full">
            ...
            <input
              ...
              value={pt.busqueda}
              onChange={(e) => pt.setBusqueda(e.target.value)}
              className="flex-1 bg-transparent text-[12.5px] text-[#1C0F05] placeholder:text-[#C0B4AA] outline-none"
            />
            {pt.busqueda && (
              <button onClick={() => pt.setBusqueda("")} className="text-[#B5A99E] hover:text-[#5A4A3C] transition-colors">
          ...
          <div className="flex bg-[#EDE8E1] rounded-lg p-1 gap-1 shrink-0">
            {(["activos", "inactivos"] as const).map((tab) => (
              <button
                key={tab}
                onClick={() => pt.setFiltroEstado(tab)}
        ...
        <div className="bg-white rounded-xl border border-[#EDE8E1] shadow-sm flex flex-col overflow-hidden">
          ...
          <div className="overflow-auto max-h-[300px]">
            <table className="w-full text-[12.5px] text-left relative">
              <thead className="sticky top-0 z-10 shadow-[0_1px_0_#EDE8E1]">
                <tr>
                  <th className="px-5 py-3 font-bold text-[10px] uppercase tracking-[0.8px] text-[#A8978B] bg-[#FDFAF7]">Transportista</th>
                  <th className="px-5 py-3 font-bold text-[10px] uppercase tracking-[0.8px] text-[#A8978B] bg-[#FDFAF7]">Documento (RUC/DNI)</th>
                  <th className="px-5 py-3 font-bold text-[10px] uppercase tracking-[0.8px] text-[#A8978B] bg-[#FDFAF7]">Teléfono</th>
              ...
              <tbody className="divide-y divide-[#F5F0EB]">
                {pt.cargando ? (
                  Array.from({ length: 5 }).map((_, i) => (
                    <tr key={i} className="animate-pulse">
                      ...
                      <td className="px-5 py-3.5"><div className="h-4 bg-[#F0EBE4] rounded w-24" /></td>
                ...
                ) : pt.transportistasPaginados.length === 0 ? (
                  <tr>
                    <td colSpan={5} className="px-5 py-12 text-center text-[#9A8E82] text-sm">
                      {pt.busqueda
                        ? `Sin resultados para "${pt.busqueda}".`
                        : `No hay transportistas ${pt.filtroEstado}.`}
                    </td>
                  </tr>
                ) : (
                  pt.transportistasPaginados.map((transportista) => (
                    <tr key={transportista.id} className="group hover:bg-[#FDFAF7] transition-colors">
                      ...
                      <td className="px-5 py-3">
                        {transportista.ruc_dni ? (
                          <div className="flex items-center gap-1.5">
                            <CreditCard className="w-3.5 h-3.5 text-[#B5A99E]" />
                            <span className="text-[11.5px] font-mono text-[#5A4A3C] font-medium">{transportista.ruc_dni}</span>
                          </div>
                        ) : (
                      ...
                      <td className="px-5 py-3">
                        <div className="flex items-center justify-end gap-1.5 opacity-100 lg:opacity-0 lg:group-hover:opacity-100 transition-opacity">
                          {transportista.estado ? (
                            <>
                              <button
                                onClick={() => pt.abrirModalEditar(transportista)}
                              ...
                              <button
                                onClick={() => pt.handleDesactivar(transportista.id, transportista.nombre)}
                            ...
                            <button
                              onClick={() => pt.handleReactivar(transportista.id)}
          ...
          <div className="px-5 py-2.5 bg-[#FDFAF7] border-t border-[#EDE8E1] flex items-center justify-between shrink-0">
            <span className="text-[11px] text-[#9A8E82]">
              {pt.transportistasFiltrados?.length ?? 0} transportista
              {(pt.transportistasFiltrados?.length ?? 0) !== 1 ? "s" : ""}
            </span>
            <Paginador />
          </div>
        </div>
      ...
      {pt.isModalOpen && (
        <div className="fixed inset-0 z-50 flex items-end sm:items-center justify-center">
          <div
            ...
            onClick={pt.cerrarModal}
          />

          <div className="relative bg-white w-full max-w-[500px] mx-0 sm:mx-4 rounded-t-2xl sm:rounded-2xl shadow-2xl flex flex-col max-h-[95vh] sm:max-h-[88vh] border border-[#EDE8E1]">
            ...
            <div className="shrink-0 flex items-start justify-between px-5 py-4 border-b border-[#F0EBE4]">
              <div>
                <h2 className="text-[15px] font-black text-[#1C0F05]">
                  {pt.transportistaAEditar ? "Editar transportista" : "Nuevo transportista"}
                </h2>
                <p className="text-[11px] text-[#9A8E82] mt-0.5">
                  {pt.transportistaAEditar
                    ? "Actualiza los datos del transportista."
                    : "Registra una nueva empresa o persona de transporte."}
                </p>
              </div>
              <button
                onClick={pt.cerrarModal}
            ...
            <form
              id="transportistaForm"
              onSubmit={pt.handleSubmit}
            ...
            >
              ...
              <section>
                ...
                <div className="space-y-4">
                  <div>
                    <FieldLabel required>Razón Social / Nombre Completo</FieldLabel>
                    <input
                      type="text"
                      value={pt.formData.nombre}
                      onChange={(e) => pt.handleChange("nombre", e.target.value)}
                      placeholder="Ej. Transportes del Norte S.A.C."
                      className={`${inputBase} ${pt.errores.nombre ? inputError : ""}`}
                    />
                    {pt.errores.nombre && (
                      <p className="text-red-500 text-[10.5px] mt-1 font-medium">{pt.errores.nombre}</p>
                    )}
                  </div>

                  <div className="grid grid-cols-2 gap-4">
                    <div>
                      <FieldLabel hint="Opcional">RUC o DNI</FieldLabel>
                      <input
                        type="text"
                        value={pt.formData.ruc_dni}
                        onChange={pt.handleDocumentoChange}
                        placeholder="Número de documento"
                        className={`${inputBase} ${pt.errores.ruc_dni ? inputError : ""}`}
                      />
                      {pt.errores.ruc_dni && (
                        <p className="text-red-500 text-[10.5px] mt-1 font-medium">{pt.errores.ruc_dni}</p>
                      )}
                    </div>
                    
                    <div>
                      <FieldLabel hint="Opcional">Teléfono de contacto</FieldLabel>
                      <input
                        type="tel"
                        value={pt.formData.telefono}
                        onChange={pt.handleTelefonoChange}
                        placeholder="Ej. 987654321"
                        className={`${inputBase} ${pt.errores.telefono ? inputError : ""}`}
                      />
                      {pt.errores.telefono && (
                        <p className="text-red-500 text-[10.5px] mt-1 font-medium">{pt.errores.telefono}</p>
                      )}
                    </div>
                  </div>
                </div>
              ...
              >
                {pt.guardando ? (
                  ...
                  <>
                    ...
                    {pt.transportistaAEditar ? "Actualizar registro" : "Registrar transportista"}
                  </>
                )}
              </button>
            </div>
